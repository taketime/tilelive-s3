...
before(function(done) {
    new TileJSON('tilejson://' + __dirname + '/fixtures/world-bright.s3', function(err, source) {
        world_bright = source;
        done(err);
    });
});
...
var grid_source;
before(function(done) {
    new TileJSON('tilejson://' + __dirname + '/fixtures/grid.s3', function(err, source) {
        grid_source = source;
        done(err);
    });
});
...
describe('loading bad tilejson files', function() {
    it('should return ENOENT for missing file', function(done) {
         new TileJSON('tilejson://' + __dirname + '/fixtures/enoent.s3', function(err, source) {
            assert.ok(err);
    ...
    it('should return parser error for invalid JSON', function(done) {
         new TileJSON('tilejson://' + __dirname + '/fixtures/bad.s3', function(err, source) {
            assert.ok(err);
...
describe('loading tiles from bad server', function() {
    ...
    before(function(done) {
        new TileJSON('tilejson://' + __dirname + '/fixtures/invalid.s3?timeout=200', function(err, source) {
            tilejson = source;
            done(err);
        });
    });
