...
S3.prototype.putTile = function(z, x, y, data, callback) {
    ...
    if (this._cachePacks.length > 256 * (this.data.pack||1)) {
        this._flush();
        return callback();
    } else {
        return callback();
...
S3.prototype._flush = function(callback) {
    ...
    if (this._flushing >= 5)
        console.warn('WARNING: Flushing %s sets concurrently.', this._flushing);

    var remaining = Object.keys(packs).length;

    var complete = function() {
        if (!--this._flushing) this.emit('flush');
