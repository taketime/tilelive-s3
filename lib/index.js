...
S3.prototype.putTile = function(z, x, y, data, callback) {
    ...
    if (Object.keys(this._cacheTiles).length >= 64 && !(key in this._cacheTiles)) {
        this._flush(this._cacheTiles, callback);
        this._cacheTiles = {};
...
S3.prototype._flush = function(packs, callback) {
    // If no packs to put, we're done.
    if (!Object.keys(packs).length) return callback();

    // Lock flushes.
    if (this._flushing) return this.once('flush', function() {
        this._flush(packs, callback);
    }.bind(this));
    ...
    var put = function(key, md5, buffer, done, retries) {
        retries = retries || 0;
        if (Object.keys(s3._cacheRefs).length > 1000) s3._cacheRefs = {};
    ...
    var get = function(key, tiles, done) {
        s3._loadTile(tiles[0].z, tiles[0].x, tiles[0].y, function(err, buffer) {
            ...
            if (s3.data.pack) {
                // Clear out new buffers as they could contain garbage image
                // data from other discarded buffers.
...
S3.prototype.stopWriting = function(callback) {
    ...
    this._flush(this._cacheTiles, function() {
