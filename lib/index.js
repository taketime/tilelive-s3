...
S3.prototype.putTile = function(z, x, y, data, callback) {
    ...
    // Flush interval here attempts to respond to pack size intelligently,
    // e.g. do roughly 256 concurrent puts at once.
    if (this._cachePacks.length > 256 * (this.data.pack||1)) this._flush();
...
S3.prototype._flush = function(callback) {
    ...
    var packs = this._cachePacks.reduce(function(memo, t) {
        if (this.data.packSize && (t.data.length > this.data.packSize))
            console.warn('Tile %s/%s/%s %s exceeds packSize %s', t.z, t.x, t.y, t.data.length, this.data.packSize);
    ...
    var put = function(key, md5, buffer, retries) {
        retries = retries || 0;
        ...
        s3._loadTile(tiles[0].z, tiles[0].x, tiles[0].y, function(err, buffer) {
            ...
            if (s3.data.pack) {
                if (err || buffer.length !== s3.data.pack * s3.data.packSize)
                    buffer = new Buffer(s3.data.pack * s3.data.packSize);

                tiles.forEach(function(tile) {
                    tile.data.copy(buffer, s3._packRange(tile.z,tile.x,tile.y)[0]);
                });
    ...
    for (var key in packs) get(key, packs[key]);
};
...
S3.prototype.stopWriting = function(callback) {
    console.warn('');
    console.warn('S3 final flush (%s tiles)...', this._cachePacks.length);
