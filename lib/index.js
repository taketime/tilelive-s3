...
S3.prototype._prepareURL = function(url, z, x, y) {
    if (this.data.pack) {
        var pz = ((z / this.data.pack) | 0) * this.data.pack;
        x = x >> (z - pz);
        y = y >> (z - pz);
        z = pz;
    }
...
S3.prototype._packRange = function(z, x, y) {
    ...
    var pz = ((z / this.data.pack) | 0) * this.data.pack;
    var side = Math.pow(2, z - pz);
    var i = _(z - pz).range().reduce(function(memo, i) {
        memo += Math.pow(4,i);
        return memo;
    }, (((y|0) % side) * side) + ((x|0) % side));
...
S3.prototype._loadTile = function(z, x, y, callback) {
    ...
    if (range) headers.Range = 'bytes=' + range.join('-');
    new get({
        uri:url,
