...
S3.prototype._loadTile = function(z, x, y, callback, all) {
    ...
    console.time('get ' + [z,x,y].join(','));
    new get({
        uri:url,
        timeout:this.timeout,
        headers:headers
    }).asBuffer(function(err, data, headers) {
        console.timeEnd('get ' + [z,x,y].join(','));
        // for copy
        // if (err) return callback(null, new Buffer(1024));
        if (err) return callback(err.status === 404 ? new Error('Tile does not exist') : err);
        if (data[0] === 0) return callback(new Error('Tile does not exist'));

        var modified = headers['last-modified'] ? new Date(headers['last-modified']) : new Date;
        var responseHeaders = {
            'Type-Content': getMimeType(data),
...
S3.prototype._loadPack = function(z, x, y, callback) {
    return this._loadTile(z,x,y,callback,true);
};
...
S3.prototype.putTile = function(z, x, y, data, callback) {
    // Just cache. All puts are done at stopWriting as proof of concept.
...
S3.prototype.stopWriting = function(callback) {
    ...
    console.warn(Object.keys(packs));
    ...
    var done = function() {
        ...
        console.log('--------');
        console.log('GET:   %s', stats.get);
        console.log('PUT:   %s', stats.put);
        console.log('No-op: %s', stats.noop);
        console.log('');
        return callback();
    };
    for (var key in packs) {
        var tiles = packs[key];
        var buffer = new Buffer(this.data.pack * this.data.packSize);
